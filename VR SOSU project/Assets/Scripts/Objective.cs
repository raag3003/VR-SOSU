...
namespace Core.Gameplay.Objectives
{
    public class Objective
    {
        ...
        {
            // Example objective
            Objective findHansObjective = new Objective("FindHans", "Hans Found: {0}/{1}", 1);
            objectiveManager.AddObjective(findHansObjective);
            findHansObjective.AddProgress(1); // adds +1 progress ONLY to findHansObjective
            // objectiveManager.AddProgress("FindHans", 1); // adds +1 progress to all objectives with the event trigger "FindHans"
        }
